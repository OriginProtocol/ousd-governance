...
def test_proposal_can_be_queued_and_executed_in_timelock(governance, staking, token, timelock_controller, web3):
    alice = accounts[0]
    amount = 1000 * 10 ** 18
    token.approve(staking.address, amount * 10, {"from": alice})
    staking.stake(amount, WEEK, alice, {"from": alice})
...
def test_late_vote_extends_quorum(governance, staking, token, timelock_controller, web3):
    alice = accounts[0]
    amount = 1000 * 10 ** 18
    token.approve(staking.address, amount * 10, {"from": alice})
    staking.stake(amount, WEEK, alice, {"from": alice})
...
def test_timelock_proposal_can_be_cancelled(governance, staking, token, timelock_controller, web3):
    alice = accounts[0]
    amount = 1000 * 10 ** 18
    token.approve(staking.address, amount * 10, {"from": alice})
    staking.stake(amount, WEEK, alice, {"from": alice})
...
def test_timelock_proposal_can_be_cancelled_after_time_passes(governance, staking, token, timelock_controller, web3):
    alice = accounts[0]
    amount = 1000 * 10 ** 18
    token.approve(staking.address, amount * 10, {"from": alice})
    staking.stake(amount, WEEK, alice, {"from": alice})
...
def test_timelock_proposal_can_not_be_cancelled_after_is_executed(governance, staking, token, timelock_controller, web3):
    alice = accounts[0]
    amount = 1000 * 10 ** 18
    token.approve(staking.address, amount * 10, {"from": alice})
    staking.stake(amount, WEEK, alice, {"from": alice})
