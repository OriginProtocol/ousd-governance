...
const { verifyContractWithArgs, verifyContractNoArgs, week } = require("./helpers");
 ...
 */
async function verify(networkId, rewardsStartTimestamp, mandatoryEndBlock, optionalEndBlock) {
  ...
  try {
    await verifyContractWithArgs(deployment.veogvImpl, deployment.ogv, rewardsStartTimestamp, week, deployment.rewardsSource);
  } catch (err) {
    console.log(err.message);
  }
  try {
    await verifyContractWithArgs(deployment.timelock, [admin.address], [admin.address]);
  } catch (err) {
    console.log(err.message);
  }
  try {
    await verifyContractWithArgs(deployment.governance, deployment.veogv, deployment.timelock);
  } catch (err) {
    console.log(err.message);
  }
  // Load merkle claims
  ...
  try {
    await verifyContractWithArgs(deployment.optionalLockup, deployment.ogv, 
        optionalLockupClaims.merkleRoot, deployment.veogv, optionalEndBlock);
  } catch (err) {
    console.log(err.message);
  }
  try {
    await verifyContractWithArgs(deployment.mandatoryLockup, deployment.ogv, 
        mandatoryLockupClaims.merkleRoot, deployment.veogv, mandatoryEndBlock);
  } catch (err) {
    console.log(err.message);
  }
