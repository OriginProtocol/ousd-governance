...
const { deploy, deployArgs, toHex } = require('../scripts/helpers');
 ...
 */
async function deployContracts(networkId, rewardsStartTimestamp, mandatoryEndBlock, optionalEndBlock) {
    ...
    const veogvImpl = await deployArgs('OgvStaking', ogv.address, rewardsStartTimestamp, minStakingTime, rewardsSource.address);
    ...
    // Deploy timelock
    const timelock = await deployArgs('Timelock', [admin.address], [admin.address]);
    ...
    const optionalLockup = await deployArgs('OptionalLockupDistributor', 
      ogv.address, optionalLockupClaims.merkleRoot, veogv.address, optionalEndBlock);
    
    const mandatoryLockup = await deployArgs('MandatoryLockupDistributor', 
      ogv.address, mandatoryLockupClaims.merkleRoot, veogv.address, mandatoryEndBlock);
    ...
    fs.writeFileSync(`./build/deployments/deployment_${networkId}.json`, JSON.stringify(deployment));

    // Verify the contracts
    const networkName = hre.network.name
    if(networkName !== 'hardhat') {
      await verify(networkId, rewardsStartTimestamp, mandatoryEndBlock, optionalEndBlock)
    }
}